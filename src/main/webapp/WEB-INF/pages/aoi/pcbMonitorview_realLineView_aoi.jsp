	...
	<body>
        ...
        window.operateEventsRealLineView={
            "click #MachineListNo" :function(e,value, row, index){
                window.open("${basePath}/aStatus/historicalList_aoi?lineNo="+row.lineNo+"&aoiMode="+aoiType);
            },
            "click #MachineListNo_i" :function(e,value, row, index){
                window.open("${basePath}/aStatus/historicalList_aoi?lineNo="+row.lineNo+"&aoiMode="+aoiType);
            }
        };
        var StatusQueryUrl = '${basePath}/aStatus/pcbMonitorJson';
        var vValue=1;
        ...
        function FPYRealTime(value,x){
           ...
           // alert(value+mode);
            var json = {};
            $.ajax({
                url: "${basePath}/aStatus/pcbMonitorview_realLineViewJson?aValue="+value+"&mode="+mode+"&aoiType="+aoiType,
        ...
        function InitMainTableLeft () {
            ...
            $table = $('#machineStatusLeft').bootstrapTable({
                url: StatusQueryUrl+"?lane="+0 +"&aoiType="+aoiType,                      //请求后台的URL（*）
                dataType:"json",
                method: 'GET',                      //请求方式（*）
                //data:{lane:0},
        ...
        function InitMainTableRight(){
             $('#machineStatusRight').bootstrapTable({
                url: StatusQueryUrl+"?lane="+1+"&aoiType="+aoiType,                        //请求后台的URL（*）
                dataType:"json",
                method: 'GET',                      //请求方式（*）
                //data:{lane:1},
                //toolbar: '#toolbar',              //工具按钮用哪个容器
        ...
        function refreshTableLeft(){
			var opt = {
				url: StatusQueryUrl+"?lane="+0+"&aoiType="+aoiType,
				silent: true,
				query:{
					type:1,
					level:2
        ...
        function refreshTableRight(){
            var opt = {
                url: StatusQueryUrl+"?lane="+1+"&aoiType="+aoiType,
                silent: true,
                query:{
                    type:1,
                    level:2
